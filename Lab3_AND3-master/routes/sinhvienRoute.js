...
router.post('/sinhvien',async (req,res)=>{
    try {
        const {id,tensv}=req.body;//lay du lieu nguoi dung nhap tu React Native
        const sinhvien1=new sinhvien({id,tensv});//tao doi tuong moi voi du lieu user nhap
...
router.put('/sinhvien/:_id', async (req,res)=>{
    try {
        ...
        const {id,tensv}=req.body;//lay du lieu nguoi dung nhap tu React Native
        const updateSinhVien=await sinhvien.findByIdAndUpdate(_id,{id,tensv},{new: true});
